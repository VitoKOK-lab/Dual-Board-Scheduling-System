...
CREATE TABLE IF NOT EXISTS location_tasks (
    ...
    board_type         TEXT    NOT NULL CHECK (board_type IN ('WHITEBOARD', 'BLACKBOARD', 'SPECIAL')),
    shift_type         TEXT    NOT NULL CHECK (shift_type IN ('MORNING', 'FLAG', 'NOON', 'ALL_WEEK', 'DAILY', 'SPECIAL')),
...
CREATE TABLE IF NOT EXISTS fairness_stats (
    ...
    special_count            INTEGER NOT NULL DEFAULT 0   -- 公差（隊裡的特殊任務）
);
...
CREATE TABLE IF NOT EXISTS weekly_schedules (
    ...
    flag_days       TEXT    NOT NULL DEFAULT '',          -- 本週的升旗日，如 '1,4'；空字串代表沒有升旗
    generated_at    TEXT,
    published_at    TEXT
);
...
-- ---------------------------------------------------------------
-- 以下為 §4.5（發布結算）所必需，屬 §3 的實作補充。
-- ---------------------------------------------------------------

-- 6. 公平性結算帳本：記錄每次「發布」對 fairness_stats 的增量，
...
CREATE TABLE IF NOT EXISTS fairness_ledger (
    ...
    special_delta    INTEGER NOT NULL DEFAULT 0,
